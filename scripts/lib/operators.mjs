...
//
// Why 200, not 20: the architecture applies at most ONE operator
// per iteration, and pairwise ops (DESCEND/LIFT/MERGE) always win
// priority over cluster NEST (tryClusterNestIteration runs only
// when no pairwise op fires in a given iteration — see the while
// loop in `runConvergence`). On a 596-leaf hand-authored corpus
// observed in the field (skill-code-review/reviewers.src), Tier 1
// similarity produces ~20 viable MERGE pairs; the old cap of 20
// iterations burned entirely on those MERGEs and cluster NEST —
// where the Phase X.10 coarse-partition emits ~75 top-level NESTs
// — never got a chance to run. Result: the downstream balance
// phase saw a 576-leaf root, tried linear carving, hit its own
// 20-iter cap, and the whole build rolled back.
//
