...
- `tests/unit/balance.test.mjs` — 14 new scenarios: depth-map, max-depth, fanout-stats, overload detection (with and without `nestedParents` exclusion, plus a leaf-metric regression guard against flagging dirs overfull only via subdir count), depth-overage detection (only single-child passthroughs), flatten happy path + refuse on multi-child, `runBalance` no-op when flags absent, fanout-only pass, depth-only pass, fanout pass skips un-actionable `overfull[0]` and acts on a later candidate, multiplier constant pin.
